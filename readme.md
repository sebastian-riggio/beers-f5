...
Since beer is one of the most consumed drinks by some of the FactoriaF5 hackers 🍻 , our mission here is to create an app to showcase some of the best handcrafted beers, but not just that - to save some as well so the rest of the coders community is informed 😌. Our end goal is to create something like this:

<div style="display: flex; justify-content: center">
<img src="https://imgur.com/6rsO424.gif" />
</div>
...
This boiler-plated code is equipped with unit tests to provide automated feedback on your progress and help you understand whether your code is working as expected. If you want to check the tests, they are located in the `src/test` folder.
...
The **BeersF5** app will include the following features:
...
1. Set up React Router in your `src/main.tsx` file:
2. Includes de following routes in App.tsx:
...
- Route to a `NotFoundPage` component
<br>
...
Implement the `HomePage` component located in the `src/pages/HomePage.tsx` so that it includes links to the following pages:
...
<div style="display: flex; justify-content: center">
  <img src="https://imgur.com/vMNckBV.png" height="600px" />
</div>
...
In this iteration, you will work on the `Navbar` component located in `src/components/Navbar.tsx`.
...
<div style="display: flex; justify-content: center">
  <img src="https://imgur.com/dXINwvV.png" height="100px" />
</div>
...
Next, we'll work on the `AllBeersPage` component in the `src/pages/AllBeersPage.tsx`.

The `AllBeersPage` that gets rendered on the route `/beers`, should display a list of all the beers from the API.



To do this, you need to make a `GET` request to the Beers API endpoint `https://f5-beers-065cad3017be.herokuapp.com/beers`. This API endpoint returns an **array of beers**.
...
<div style="display: flex; justify-content: center">
  <img src="https://imgur.com/PnvQpaY.png" height="600px" />
</div>
...
In this iteration, you will work on the `BeerDetailsPage` component in the `src/pages/BeerDetailsPage.tsx`.
...
Check [this example](https://reactrouter.com/en/main/hooks/use-params) if you need a reminder of how to set up the useParams hook and access the URL parameters.
...
When the `RandomBeerPage` component is rendered on the `/random-beer` route, it should show a random beer retrieved from the Beers API. To get the data of a random beer, you need to make a `GET` request to the endpoint `https://f5-beers-065cad3017be.herokuapp.com/beers/random`.
...
In this iteration, you will work on the `AddBeerPage` component in the `src/pages/AddBeerPage.tsx`.
...
Once you are done creating the form, make a `POST` request to the API endpoint `https://f5-beers-065cad3017be.herokuapp.com/beers/new`, passing all the input values in the request `body` as an object. The fields of the request `body` should have exact **names** so that the API can create a new beer.
...
In the `AllBeersPage` component, add an `input` where users can type in their search query. Every time the user types a new letter, you should call to `https://f5-beers-065cad3017be.herokuapp.com/beers/search?q={query}`, passing the input value as the `q` param.
...
<details>
  ...
  **Important**: You should not use *index* of the `map` as *key*. This is considered an *anti-pattern* that may lead to unpredictable results.
  ...
  For more information, check: [React Docs - Rendering Lists](https://react.dev/learn/rendering-lists#keeping-list-items-in-order-with-key)
...
const projects = [
  ...
  **Important**: You should not use *index* of the `map` as *key*. This is considered an *anti-pattern* that may lead to unpredictable results.
  ...
  For more information, check: [React Docs - Rendering Lists](https://react.dev/learn/rendering-lists#)
